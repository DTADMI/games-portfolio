...
export function useStomp(opts?: { url?: string; enabled?: boolean; headers?: Record<string, string> }) {
  const enabled = opts?.enabled ?? (process.env.NEXT_PUBLIC_FEATURE_REALTIME === "true");
  const url = opts?.url ?? (typeof window !== "undefined" ? `${window.location.origin.replace(/^http/, 'ws')}/ws` : "");
  ...
  useEffect(() => {
    ...
    return () => {
      try { subsRef.current.forEach(s => s.unsubscribe()); } catch {}
      subsRef.current = [];
      client.deactivate();
  ...
  const api = useMemo(() => ({
    ...
    subscribe: (destination: string, cb: (msg: StompMessage) => void) => {
      ...
      const sub = clientRef.current.subscribe(destination, (m: IMessage) => {
        try { cb(JSON.parse(m.body)); } catch { /* ignore */ }
      });
