...
const PATH: { x: number; y: number }[] = [
  {x: 0, y: 5}, {x: 5, y: 5}, {x: 5, y: 2}, {x: 10, y: 2}, {x: 10, y: 8}, {x: 19, y: 8}
];
...
export const TowerDefenseGame: React.FC = () => {
  ...
  function placeTower(tx: number, ty: number) {
    ...
    for (let i = 0; i < PATH.length - 1; i++) {
      ...
      const wx = cx - a.x, wy = cy - a.y;
      const t = Math.max(0, Math.min(1, (wx * vx + wy * vy) / (vx * vx + vy * vy || 1)));
      const px = a.x + t * vx, py = a.y + t * vy;
      if (dist(cx, cy, px, py) < 0.9) return; // too close to path
    }
    if (money < 50) return;
  ...
  function step(dt: number) {
    if (status === 'spawning') {
      ...
      if (toSpawn >= creepCount) setStatus('running');
    }
    ...
    creeps.current.forEach((c) => {
      ...
      if (!target) return; // reached end (should be handled below)
    ...
    towers.current.forEach((t) => {
      ...
      if (best) {
        ...
        shots.current.push({x: t.x, y: t.y, vx: (dx / len) * speed, vy: (dy / len) * speed, dmg: 15, alive: true});
        t.cd = 1 / t.fireRate;
    ...
    shots.current.forEach((s) => {
      ...
      // out of bounds
      if (s.x < 0 || s.y < 0 || s.x > COLS || s.y > ROWS) s.alive = false;
    });
    ...
    // wave end conditions
    if ((status === 'running' || status === 'spawning') && creeps.current.length === 0 && status !== 'spawning') {
      setWave((w) => w + 1);
      setStatus('idle');
    }
    if (lives <= 0 && status !== 'lost') setStatus('lost');
  }
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      if (e.key.toLowerCase() === 'n' && (status === 'idle' || status === 'won')) startWave();
  ...
  return (
    <div className="flex flex-col items-center justify-center p-2">
      ...
      />
      <div className="mt-2 text-sm text-gray-300">Click to place a tower ($50). Press N to start next wave, R to
        reset.
      </div>
    </div>
  );
