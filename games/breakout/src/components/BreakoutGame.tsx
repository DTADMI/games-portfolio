...
export const BreakoutGame: React.FC = () => {
    ...
    const [activePowerUps, setActivePowerUps] = useState<Record<string, { type: PowerUpType; endTime: number }>>({});
    ...
    const drawHUD = (ctx: CanvasRenderingContext2D) => {
        ...
        if (gameOver) {
            ...
            ctx.font = '24px Arial';
            ctx.fillText(`Final Score: ${score}`, canvasRef.current!.width / 2, canvasRef.current!.height / 2 + 10);

            ctx.font = '20px Arial';
            ctx.fillText('Press Space to Play Again', canvasRef.current!.width / 2, canvasRef.current!.height / 2 + 60);
            ...
            ctx.textAlign = 'center';
            ctx.textBaseline = 'middle';
            ctx.fillText('Level Complete!', canvasRef.current!.width / 2, canvasRef.current!.height / 2 - 30);

            ctx.font = '20px Arial';
            ctx.fillText('Get ready for the next level...', canvasRef.current!.width / 2, canvasRef.current!.height / 2 + 20);
        } else if (!gameStarted) {
            ctx.fillStyle = 'rgba(0, 0, 0, 0.7)';
            ctx.fillRect(0, 0, canvasRef.current!.width, canvasRef.current!.height);
            ...
            ctx.font = '20px Arial';
            ctx.fillText('Use arrow keys or A/D to move', canvasRef.current!.width / 2, canvasRef.current!.height / 2);
            ctx.fillText('Press Space to Start', canvasRef.current!.width / 2, canvasRef.current!.height / 2 + 40);
        } else if (isPaused) {
            ...
            ctx.font = '20px Arial';
            ctx.fillText('Press Space to Resume', canvasRef.current!.width / 2, canvasRef.current!.height / 2 + 50);
        }
    };
    ...
    const updateBalls = () => {
        if (!gameStarted || isPaused || balls.length === 0) return;
                    ...
                    if (newBricks[c][r].health <= 0) {
                        ...
                        if (Math.random() < 0.2) { // 20% chance
                            const powerUpTypes: PowerUpType[] = [
                                'expand', 'shrink', 'slow', 'fast', 'multiball', 'laser', 'extraLife'
                            ];
        ...
        >
            ...
            <div className="mt-6">
                ...
                <ul className="text-sm text-gray-600 dark:text-gray-300 space-y-1">
                    <li>• Use <kbd>←</kbd> <kbd>→</kbd> or <kbd>A</kbd> <kbd>D</kbd> to move the paddle</li>
                    <li>• Press <kbd>Space</kbd> to start/pause the game</li>
