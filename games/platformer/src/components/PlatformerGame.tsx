...
const MAP: number[][] = (() => {
  ...
  // some platforms
  for (let x = 3; x < 7; x++) m[ROWS - 4][x] = 1;
  for (let x = 10; x < 15; x++) m[ROWS - 6][x] = 1;
  // collectibles
  m[ROWS - 7][12] = 2;
  m[ROWS - 5][5] = 2;
  m[ROWS - 2][2] = 2;
...
export const PlatformerGame: React.FC = () => {
  ...
  // player
  const player = useRef({x: 2 * TILE, y: (ROWS - 3) * TILE, vx: 0, vy: 0, w: 24, h: 28, onGround: false});
  ...
  function step(dt: number) {
    ...
    // input
    p.vx = 0;
    if (keys.current['ArrowLeft'] || keys.current['a']) p.vx = -MOVE_VX;
    if (keys.current['ArrowRight'] || keys.current['d']) p.vx = MOVE_VX;
    ...
    if (aabbTileCollision(nx, ny, p.w, p.h)) {
      ...
      while (!aabbTileCollision(p.x + Math.sign(dir), ny, p.w, p.h)) p.x += Math.sign(dir);
      p.vx = 0;
      nx = p.x;
    }
    p.x = nx;
    ...
    if (aabbTileCollision(p.x, ny, p.w, p.h)) {
      ...
      while (!aabbTileCollision(p.x, p.y + Math.sign(dirY), p.w, p.h)) p.y += Math.sign(dirY);
      if (dirY > 0) p.onGround = true; // landing
      p.vy = 0;
      ny = p.y;
    } else {
      p.onGround = false;
  ...
  function render() {
    ...
    if (won) ctx.fillText('Goal reached! Press R to restart.', 8, 36);
  }
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      if (e.key.toLowerCase() === 'r') {
        // reset
        player.current = {x: 2 * TILE, y: (ROWS - 3) * TILE, vx: 0, vy: 0, w: 24, h: 28, onGround: false};
        setScore(0);
        setWon(false);
      }
    };
  ...
  return (
    <div className="flex flex-col items-center justify-center p-2">
      ...
      />
      <div className="mt-2 text-sm text-gray-300">Arrows/WASD to move & jump. Press R to restart.</div>
    </div>
  );
};
