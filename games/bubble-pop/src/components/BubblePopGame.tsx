...
export function findGroup(board: Board, sx: number, sy: number): Point[] {
  if (!inBounds(sx, sy, board[0].length, board.length)) return [];
  ...
  while (q.length) {
    ...
    seen.add(k);
    if (!inBounds(x, y, board[0].length, board.length)) continue;
...
export function popGroup(board: Board, group: Point[]): Board {
  ...
  for (const {x, y} of group) next[y][x] = -1;
  return next;
}
...
export function refill(board: Board): Board {
  ...
  for (let y = 0; y < next.length; y++) {
    for (let x = 0; x < next[0].length; x++) {
      if (next[y][x] < 0) next[y][x] = Math.floor(Math.random() * COLORS.length);
    }
  }
  return next;
}
...
export const BubblePopGame: React.FC = () => {
  ...
  const onClick = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    const sameGroup = selected && group.length === selected.length && selected.every((p) => group.some((g) => g.x === p.x && g.y === p.y));
    if (sameGroup) {
  ...
  useEffect(() => {
    const handler = (ev: KeyboardEvent) => {
      if (ev.key === 'ArrowRight') setCursor((c) => ({x: Math.min(COLS - 1, c.x + 1), y: c.y}));
      if (ev.key === 'ArrowLeft') setCursor((c) => ({x: Math.max(0, c.x - 1), y: c.y}));
      if (ev.key === 'ArrowDown') setCursor((c) => ({x: c.x, y: Math.min(ROWS - 1, c.y + 1)}));
      if (ev.key === 'ArrowUp') setCursor((c) => ({x: c.x, y: Math.max(0, c.y - 1)}));
    ...
    <div className="flex flex-col items-center justify-center p-2">
      ...
      <div className="mt-3 flex items-center gap-3">
        <button onClick={onRestart}
                className="px-3 py-1 rounded bg-emerald-600 text-white hover:bg-emerald-700">Restart
        </button>
        <span
          className="text-sm text-gray-400">Use mouse/touch to select groups (3+). Space/Enter to pop selected.</span>
      </div>
      <div aria-live="polite" className="sr-only">Score {score}. Best {best}.</div>
    </div>
  );
};
