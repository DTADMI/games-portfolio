...
import { GameState, Position, TETROMINOS, TETROMINO_TYPES, BOARD_WIDTH, BOARD_HEIGHT, CELL_SIZE } from '@/types/game';
...
const TetrisGame = () => {
    ...
    const moveTetromino = useCallback((direction: 'left' | 'right' | 'down' | 'rotate') => {
    ...
    }, [checkCollision, placeTetromino, rotate, gameState.gameOver, gameState.isPaused, gameState.gameStarted]);
            ...
            switch (e.key) {
                ...
                case ' ':
                    ...
                    while (!checkCollision(
                        gameState.tetromino,
                        gameState.board,
                        { ...gameState.tetromino.position, y: gameState.tetromino.position.y + 1 }
                    )) {
                        moveTetromino('down');
                    }
                    moveTetromino('down');
    ...
    }, [gameState.gameStarted, gameState.gameOver, gameState.isPaused, gameState.level, moveTetromino]);
    ...
    }, [gameState.gameOver, gameState.gameStarted, gameState.score]);
    ...
    const renderCell = (cell: string, rowIndex: number, colIndex: number) => {
        const isCurrentTetromino =
            ...
            gameState.tetromino.shape[rowIndex - gameState.tetromino.position.y]?.[colIndex - gameState.tetromino.position.x];
                            ...
                            {gameState.nextTetromino.shape.map((row, rowIndex) =>
                                row.map((cell, colIndex) => (
                                    <div
                                        ...
                                        style={{ backgroundColor: cell ? gameState.nextTetromino.color : 'transparent' }}
                                    />
                                ))
                            )}
                        </div>
